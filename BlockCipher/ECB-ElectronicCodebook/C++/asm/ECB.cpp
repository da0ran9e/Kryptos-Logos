...
vector<bitset<64>> encryptECB(vector<bitset<64>> plaintext, bitset<64> key) {
    vector<bitset<64>> encryptedData;
    
    for (size_t i = 0; i < plaintext.size(); i++) {
        bitset<64> block = plaintext[i];
        bitset<64> encryptedBlock = encryptDES(block, key);
        encryptedData.push_back(encryptedBlock);
    }
...
int main() {
    ...
    vector<bitset<64>> encryptedData = encryptECB(data, originalKey);
