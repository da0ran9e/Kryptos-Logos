...
// Function to generate a stream of random blocks using DES (for OFB)
vector<bitset<64>> generateRandomBlocks(bitset<64> key, size_t numBlocks) {
    vector<bitset<64>> randomBlocks;
    bitset<64> previousBlock = key; // Initialize with the key
    
    for (size_t i = 0; i < numBlocks; i++) {
        bitset<64> randomBlock = encryptDES(previousBlock, key);
        randomBlocks.push_back(randomBlock);
...
vector<bitset<64>> encryptOFB(vector<bitset<64>> plaintext, bitset<64> originalKey, bitset<64> IV) {
    vector<bitset<64>> encryptedData;
    
    // Generate a stream of random blocks using the IV as the key
    vector<bitset<64>> randomBlocks = generateRandomBlocks(IV, plaintext.size());
    
    for (size_t i = 0; i < plaintext.size(); i++) {
        // XOR the plaintext with the random block to get the ciphertext
        bitset<64> encryptedText = plaintext[i] ^ randomBlocks[i];
        encryptedData.push_back(encryptedText);
    }
...
int main() {
    ...
    vector<bitset<64>> encryptedData = encryptOFB(data, originalKey, initialVector);
