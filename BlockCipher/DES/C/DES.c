...
int main() {
    // Initialize the main key and key set
    unsigned char main_key[8];
    key_set key_sets[17];
    generate_key(main_key);
    generate_sub_keys(main_key, key_sets);

    // Create a sample message block (64 bits)
    unsigned char message_block[8] = {' ', 'V', 'u', 'D', 'u', 'c', 'A', 'n'};
    //memset(message_block, 0, 8); // Initialize to all zeros

    // Print the main key and subkeys
    printf("Main Key:\n");
    print_key_set(key_sets[0]);

    // Print the original message block
    printf("Original Message Block:\n");
    for (int i = 0; i < 8; i++) {
        printf("%02X", message_block[i]);
    }
    printf("\n");

    // Encrypt the message block using DES
    process_message(message_block, message_block, key_sets, ENCRYPTION_MODE);

    // Print the encrypted message block
    printf("Encrypted Message Block:\n");
    for (int i = 0; i < 8; i++) {
        printf("%02X", message_block[i]);
    }
    printf("\n");

    // Decrypt the message block using DES
    process_message(message_block, message_block, key_sets, DECRYPTION_MODE);

    // Print the decrypted message block
    printf("Decrypted Message Block:\n");
    for (int i = 0; i < 8; i++) {
        printf("%02X", message_block[i]);
    }
    printf("\n");
